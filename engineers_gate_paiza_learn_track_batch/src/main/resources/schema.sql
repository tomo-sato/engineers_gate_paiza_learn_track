...
CREATE TABLE IF NOT EXISTS `paiza_learn_track`.`team_users` (
  ...
  `learning_start_date` DATE NULL COMMENT '学習開始日',
  `github_account` VARCHAR(64) NULL COMMENT 'Githubアカウント',
...
-- -----------------------------------------------------
-- Table `paiza_learn_track`.`task_categories`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `paiza_learn_track`.`task_categories` (
  `id` BIGINT NOT NULL AUTO_INCREMENT COMMENT 'ID',
...
CREATE TABLE IF NOT EXISTS `paiza_learn_track`.`tasks` (
  `id` BIGINT NOT NULL AUTO_INCREMENT COMMENT 'ID',
  `task_categories_id` BIGINT NOT NULL COMMENT '課題カテゴリーID',
  ...
  INDEX `fk_tasks_task_categories1_idx` (`task_categories_id` ASC) VISIBLE)
ENGINE = InnoDB
COMMENT = '課題';
...
CREATE TABLE IF NOT EXISTS `paiza_learn_track`.`progress_rates` (
  ...
  INDEX `fk_progress_rates_task_categories1_idx` (`task_categories_id` ASC) VISIBLE)
ENGINE = InnoDB
COMMENT = '課題進捗率';
