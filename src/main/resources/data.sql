...
CREATE TABLE IF NOT EXISTS `paiza_learn_track`.`tasks` (
  ...
  `learning_hours` DECIMAL(6,2) NOT NULL COMMENT '学習時間（時）：算出した学習時間及び、割り当てた学習時間を登録（整数4桁、少数2桁）',
  `created` DATETIME NOT NULL COMMENT '作成日時',
  `updated` DATETIME NOT NULL COMMENT '更新日時',
  PRIMARY KEY (`id`),
  UNIQUE INDEX `id_UNIQUE` (`id` ASC) VISIBLE,
...
CREATE TABLE IF NOT EXISTS `paiza_learn_track`.`progress_rates` (
  ...
  `lesson_id` VARCHAR(128) NOT NULL COMMENT 'レッスンID',
  `lesson_name` VARCHAR(128) NOT NULL COMMENT 'レッスン名',
  `report_date` DATETIME NOT NULL COMMENT '集計日（yyyy-MM-dd）',
  ...
  UNIQUE INDEX `uk_progress_rates` (`team_users_id` ASC, `course_id` ASC, `report_date` ASC) VISIBLE)
ENGINE = InnoDB
COMMENT = '課題進捗率';
